...
# Colorear los 10 seleccionados según prioridad (mapa de calor)
scatter = plt.scatter(
...
plt.xlabel('X')
plt.ylabel('Y')
plt.title('10 puntos seleccionados con prioridad (mapa de calor)')
plt.legend()
plt.grid(True)
plt.show()
...
for (x, y), prioridad in puntos_prioridad.items():
...
# Realizar clustering
dbscan = DBSCAN(eps=15, min_samples=1)  # puedes ajustar eps según la escala de tus datos
...
for cluster_id in clusters_unicos:
    ...
    # puntos del cluster que están en puntos_sel (los conocidos)
    N_cluster_en_lista = sum(
        any(np.isclose(p[0], ps[0]) and np.isclose(p[1], ps[1]) for ps in puntos_sel)
...
puntos_ordenados = sorted(puntos_prioridad.items(), key=lambda x: x[1], reverse=True)
...
puntos_sel = np.array([p for p in puntos_sel if tuple(np.round(p, 2)) not in puntos_a_eliminar])

#---------------Fin de la zana a cambiar por quirobi----------------
#------------------------------------------------------------------------------------------


#-----Aumentar prioridad basal a los puntos restantes -----
aumento_prior = 0.2 # aumentamos en un 20%
